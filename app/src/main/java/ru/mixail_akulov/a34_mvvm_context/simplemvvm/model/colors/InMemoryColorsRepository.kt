...
class InMemoryColorsRepository(
    ...
    private val currentColorFlow = MutableSharedFlow<NamedColor>(
        replay = 0, // не отправлять события новым подписчикам, которые были отправлены до подписки
        extraBufferCapacity = 1, // мин. емкость буфера для использования политики переполнения DROP_OLDEST
        onBufferOverflow = BufferOverflow.DROP_OLDEST // самый новый элемент заменит самый старый элемент в случае переполнения буфера
    )
